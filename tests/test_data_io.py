...
def test_is_physics_paper_positive():
    """Real physics/SciML terms trigger the filter."""
    import data_io
    assert data_io.is_physics_paper("Physics-informed neural networks solve PDEs.")
    assert data_io.is_physics_paper("DFT surrogate models for electronic structure.")
    assert data_io.is_physics_paper("Deep learning for cosmological simulations with dark matter.")


def test_is_physics_paper_negative():
    """Non-physics text doesn't match."""
    import data_io
    assert not data_io.is_physics_paper("LLM for code generation.")
    assert not data_io.is_physics_paper("Robot navigation in unknown environments.")
    assert not data_io.is_physics_paper("")
...
def test_load_keyword_queries_raw_passthrough(tmp_path):
    ...
    assert '"Physics Informed Neural Networks"' in q
    assert '("Diffusion Models" AND (physics OR scientific OR PDE))' in q
    assert " OR " in q
    # No double-quote around the RAW expression
    assert '"RAW:' not in q
    assert '"Diffusion Models" AND' in q
