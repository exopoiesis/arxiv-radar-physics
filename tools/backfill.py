...
Usage:
    python tools/backfill.py --from-date 2024-05-01 --to-date 2026-05-03
    python tools/backfill.py --dry-run                        # one shard, no save
    python tools/backfill.py --topics "Scientific Machine Learning & PINNs"
"""
import argparse
import json
import logging
    ...
    for idx, (topic, year, month) in enumerate(shards, 1):
        ...
        for r in results:
            ...
            if pid in pid_to_month:
                ...
                continue
            relevant = is_physics_paper(rec.get("abstract", ""))
            wl_note = None
            if not relevant:
