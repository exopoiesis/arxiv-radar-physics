#!/usr/bin/env python3
"""Apply physics-context filter to existing corpus locally.
...
paper's abstract is scanned for physics/SciML-relevant keywords; non-matching
...
import data_io
from data_io import is_physics_paper  # noqa: F401  (re-export for tests/CLI)

# Backwards-compatible aliases for old callers.
...
def run(out_dir):
    ...
    for month, papers in sorted(by_month.items()):
        ...
        for pid, rec in papers.items():
            if is_physics_paper(rec.get("abstract", "")):
                kept[pid] = rec
